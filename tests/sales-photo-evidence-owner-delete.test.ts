...
runTest('capability probe is read-only and reports the route gate', async () => {
...
runTest('portable client capability check fails closed', async () => {
  const previousBaseUrl = process.env.NEXT_PUBLIC_API_BASE_URL;
  process.env.NEXT_PUBLIC_API_BASE_URL = 'https://api.example.test';
  try {
    const enabled = await getSalesPhotoEvidenceOwnerDeleteCapability({
      fetchImpl: async () => Response.json({ ok: true, enabled: true }),
    });
    const disabled = await getSalesPhotoEvidenceOwnerDeleteCapability({
      fetchImpl: async () => Response.json({ ok: true, enabled: false }),
    });
    const unavailable = await getSalesPhotoEvidenceOwnerDeleteCapability({
      fetchImpl: async () => Response.json({ ok: false }, { status: 503 }),
    });

    assert.deepEqual(enabled, { status: 'enabled' });
...
runTest('owner UI confirms deletion and updates the shared album state', () => {
  ...
  assert.match(shell, /confirmationText=\{marketRequiresEvidence \? '永久移除' : undefined\}/);
  assert.match(shell, /系統目前不會自動建立待補拍任務/);
  assert.match(shell, /成交紀錄、營收及統計不會改變/);
  assert.match(shell, /deleteCapability\?\.status === 'enabled'/);
  assert.match(detail, /setOwnerSalesPhotoEvidenceRows\(rows => rows\.filter\(row => row\.id !== evidenceId\)\)/);
  assert.match(detail, /getSalesPhotoEvidenceOwnerDeleteCapability/);
