...
async function run(): Promise<void> {
  ...
  const { ensureScheduledMarkets, skipScheduledOccurrence } = await import('../lib/recurring-operations');
  await db.open();
  await recordEvent('venue_created', { venueId: 'v', name: '固定店', status: 'active' }, 'v-event');
  await recordEvent('operation_schedule_created', {
    scheduleId: 's', venueId: 'v', timezone: 'Asia/Taipei', recurrence: { frequency: 'weekly', interval: 1, weekdays: [5], startDate: '2026-08-14' },
  ...
  const marketId = first.createdMarketIds[0]!;
  await skipScheduledOccurrence(marketId, { ownerId: 'local', isOwner: true });
  const skipped = await db.markets.get(marketId);
  assert.equal(skipped?.scheduleOccurrenceState, 'skipped');
  ...
  assert.equal(await db.markets.count(), 8);
  db.close(); await Dexie.delete('MarketPulseDB');
}
run().then(() => console.log('recurring operations skip tests passed')).catch(error => { console.error(error); process.exitCode = 1; });
