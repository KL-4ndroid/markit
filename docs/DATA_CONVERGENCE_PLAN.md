...
更新日期：2026-06-12
狀態：C2.14 資料存取盤點已完成；C2.15 Active Event Service 已建立，尚未接入 UI
...
| C2.16 | Market Projection Service | 從 active events 重建單一 market projection | `market-projection-service.ts` + tests | 中 | P0 |
| C2.17 | Recovery 接入 Projection Rebuild | 讓 `/recovery` 只用 projection service 修本機統計 | UI dry-run / execute | 中 | P0 |
| C2.18 | Sync 後 Reconciliation | sync 完成後檢查 touched markets 並自動重建不一致 projection | sync reconciliation hook | 中高 | P1 |
