...
- A revoke/rejoin cycle can leave a local sale event sync marker stale even though the sale already exists in Cloud. The local fix is verified by tests but is not yet deployed; until it is deployed, the retained 02:19 photo cannot complete its retry from the staff device and the owner album under-counts that sale's missing evidence.
