...
> 變更內容：把 Market Pulse 內部色票（霧藍 / 暖木 / 米白）對齊到「出攤本 BoothBook」對外品牌色（霧松綠 / 暖杏橘 / 奶油米白）

---

## 為什麼要對齊

原本 `.cursorrules` 與 `tailwind.config.ts` 用的是 Market Pulse 內部色票（`#7B9FA6` 霧藍 / `#D4A574` 暖木 / `#FAFAF8` 米白）。
