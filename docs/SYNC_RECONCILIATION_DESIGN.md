...
| 路徑 | 函式 | 角色 | 是否需要 reconciliation | 說明 |
|---|---|---|---:|---|
| Owner 全量 / 增量事件同步 | `pullAllEvents` | Owner | 是 | 匯入 cloud events 後可能 replay 或 skip |
| Owner snapshot 後增量 | `pullIncrementalEvents` | Owner | 是 | snapshot projection 可能污染，增量後需比對 |
| Staff view 同步 | `pullEventsFromViews` / `syncEventsToIndexedDB` | Staff | 是 | Staff 全量拉 view，但 existing event skip 可能留下 stale projection |
| Snapshot load | `loadSnapshot` 或其呼叫處 | Owner | 是 | snapshot tables 可能直接帶入 stale projection |
| Push local events | `pushEvents` | Owner / Staff | 否，暫不接 | push 不一定改本機 projection；本機 handler 已在 recordEvent 時執行 |
...
1. 先新增 `lib/sync/projection-reconciliation.ts`。
2. 測試 reconciliation helper 的狀態判斷與錯誤隔離。
3. 再逐步接入 Owner sync。
4. 最後接入 Staff sync existing-event skip 場景。

第一版只自動修 `inflated` projection，避免誤修本機低於 events 或權限不足造成的差異。
