...
> ⚠️ **重要**：需確認 `market_members` 有 RLS，確保 staff 無法直接刪除或插入自己的 `owner` 角色記錄。
...
> ⚠️ **重要**：`staff_accessible_markets` 視圖依賴 `markets` 表的 RLS，需確認 staff 無法繞過視圖直接查詢。
...
- Section 3 所有 RLS 政策是否存在且正確
- 各資料表（`markets`、`products`、`events`、`profiles`、`market_members`、`staff_invitations`）的 RLS 政策
