...
| `PWA-WEB` | Manifest assets, service-worker lifecycle, install presentation, responsive shell, and app shortcuts work | `pending_external` | 2026-08-03 local production baseline on `097f2be` passed the commit-bound 9-asset smoke, zero-error standalone manifest parsing, and no-overflow desktop 1440x900 plus Android-class 412x915 views; the unauthenticated welcome screen exposed zero `main` landmarks | Correct the welcome landmark, then complete real desktop/Android install and launch, update activation, owner/staff shortcut smoke, and public-data-only install screenshots against one HTTPS release candidate |
