...
| --- | --- | --- | --- | --- |
| `CI-WEB` | Every PR and `main` push runs hex, lint, complete tests, secret-free production build, and clean-diff checks | `complete` | Push run `30697994921` passed both Node 24 jobs and every step on commit `9aab9bec054c002c5dc924a8c33c6a6d6d0435a2` on 2026-08-01 | Preserve the gate and repeat it on the final release candidate revision |
| `LOCAL-QUALITY` | Current worktree passes lint, complete tests, production build, and `git diff --check` | `complete` | 2026-08-01 local full lint, 4m48s complete manifest, production build, hex/diff checks, and changed-file credential-pattern scan passed on `9aab9be`; the exact revision also passed CI on required Node 24 | Repeat on release candidate revision |
...
| `PROD-SURFACE` | Debug pages and dev subscription API are unreachable while the static fake-data demo remains public | `complete` | 2026-08-01 stable Production alias passed the commit-bound debug 404, dev API 404, and public demo smoke on `9aab9be` | Preserve the smoke and repeat it on the final release candidate revision |
| `DEPLOY-IDENTITY` | Remote smoke proves it reached the intended release revision | `complete` | Vercel status target `2YkYAWvAbcgp8C1ii77FS8NdWtvg` succeeded; stable alias health and four remote smokes matched `9aab9be`, version `0.1.0`, build time `2026-08-01T11:35:39.830Z` | Preserve deployment/health identity and repeat it on the final release candidate revision |
| `SECURITY-HEADERS` | Pages and APIs receive the reviewed baseline without breaking PWA/media/auth flows | `pending_external` | 2026-08-01 commit-bound Production API, page, legal, and PWA smokes passed exact headers on `9aab9be`; prior browser service worker, console, responsive shell, and single-main checks passed | Unrelated-origin HTTPS anti-frame evidence and final release-candidate repeat |
| `PWA-WEB` | Manifest assets, service-worker lifecycle, install presentation, responsive shell, and app shortcuts work | `pending_external` | 2026-08-01 commit-bound Production smoke on `9aab9be` passed 9 unique assets, manifest, service worker, and demo; prior browser checks remained controlled and responsive with one main landmark | Real desktop/Android-class install and launch, update activation, owner/staff shortcut smoke, install screenshots |
