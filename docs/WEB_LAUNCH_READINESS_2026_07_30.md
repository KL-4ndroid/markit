...
| `DB-066` | F3A private candidate price catalog foundation is live and denied to direct clients | `pending_external` | migration, read-only verifier, runbook, local static guardrail | Confirm Supabase target; apply `066`; all verifier rows true; anon/authenticated denial results recorded |
