...
| `OBSERVABILITY` | Health, callback backlog, reconciliation delay, payment failures, media cleanup, and sync incidents are observable | `implemented_local` | health route, bounded schema-v1 sales-photo events, pure 36-hour threshold evaluator, safe CLI, deterministic tests, and incident runbook; production sink, routing, sync signals, and future billing signals are absent | Production dashboards/alerts with equivalent policy, delivery proof, primary/backup owner, escalation, retention/access review, incident drill, and later S9 billing signals |
