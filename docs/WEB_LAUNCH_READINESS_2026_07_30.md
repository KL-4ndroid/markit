...
| --- | --- | --- | --- | --- |
| `CI-WEB` | Every PR and `main` push runs hex, lint, complete tests, secret-free production build, and clean-diff checks | `implemented_local` | `.github/workflows/lint.yml`; `tests/web-launch-ci.test.ts` | First GitHub Actions run passes on the pushed revision |
...
| `DB-066` | F3A private candidate price catalog foundation is live and denied to direct clients | `complete` | 2026-08-01 confirmed: subscription_price_versions, billing_storefront_price_mappings, subscription_price_assignments exist with REVOKE ALL; 5 candidate seed prices verified |
| `TEAM-LIVE` | Server-authoritative Free/Pro/Team mutation, downgrade, re-upgrade, and restore behavior is proven | `implemented_local` | 2026-08-01 subscription smoke team-enforcement passed 22/22 checks: active relationship Team backing, suspended membership cleanup, anonymous write/RPC denial, and token verification boundary | Isolated owner plus active viewer/operator/manager fixture; state-transition and cross-role smoke output |
| `PROD-CONFIG` | Production Supabase, CORS, cron, R2, media gates, and server secrets are configured | `implemented_local` | 2026-08-01 production config preflight against .env.local passed 11/21 checks; core infra (Supabase, R2, CORS, product cover routes) passing; remaining 10 failures are expected pre-launch (debug surfaces, legal, sales gates) | Passing preflight against deployed names, dated Vercel review with no values, and production route smoke |
...
| `MEDIA-PROD` | Product-cover and sales-evidence storage paths are production-ready at approved entitlement modes | `implemented_local` | 2026-08-01 R2 credentials, bucket, and endpoint pass structural validation; product-cover read/upload/delete routes and production allow gates enabled; product-cover quota set to 25 MB; sale_photo_evidence and product_cover_photos tables exist in Supabase | Migration/R2/CORS/quota/cleanup evidence and authorized/unauthorized production smoke |
