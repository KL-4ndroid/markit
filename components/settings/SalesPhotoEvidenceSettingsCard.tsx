...
export function SalesPhotoEvidenceSettingsCard() {
  ...
  useEffect(() => {
    ...
    loadDefaultSalesPhotoEvidenceRequired(user.id)
      ...
      .catch((error) => {
        console.error('載入銷售照片證明設定失敗:', error);
        toast.error('載入銷售照片證明設定失敗');
      })
      .finally(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success('銷售照片證明設定已更新');
    } catch (error: any) {
      console.error('儲存銷售照片證明設定失敗:', error);
      toast.error(error?.message || '儲存銷售照片證明設定失敗');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <section className="bg-white rounded-[1.5rem] shadow-lg shadow-primary/10 p-6">
      ...
      >
        <span>
          <span className="block text-sm font-medium text-foreground">新市集預設要求照片證明</span>
