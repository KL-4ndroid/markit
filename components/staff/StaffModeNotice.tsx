...
export function StaffModeNotice({ className = '', compact = false }: StaffModeNoticeProps) {
  ...
  return (
    <div className={`rounded-2xl border border-[#8B7BA6]/20 bg-white px-4 py-3 shadow-sm shadow-[#8B7BA6]/5 ${className}`}>
      <div className="flex items-start gap-3">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#F0ECF7] text-[#8B7BA6]">
          <Shield size={18} />
        </div>
        <div className="min-w-0">
          <p className="text-sm font-semibold text-[#3A3A3A]">員工模式</p>
          ...
          {userRole.ownerEmail && (
            <p className="mt-1 truncate text-xs text-[#8B7BA6]">
